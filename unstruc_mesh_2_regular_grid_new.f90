...
! local variables...
      real, parameter :: toler = 1.e-10, toler_visc=0.001
...
!      real dx,dy,dz
      ...
      REAL, ALLOCATABLE :: visc_vec_top(:,:,:,:), visc_vec_bot(:,:,:,:)
      real max_bot
...
! calculate the face values u from the cell centred values u_cell. 
      ...
      allocate( visc_vec_top(ndim,nx,ny,nz), visc_vec_bot(ndim,nx,ny,nz) )
...
!      print *,'i,j,k,mat_top:',i,j,k,mat_top
!      print *,'mat_bot:',mat_bot
         if(isotropic==1) then ! isotropic viscosity
            visc(:,i,j,k) = maxval(abs(mat_top(:,:)))/max(toler, 2.*maxval(abs(mat_top(:,:)))  )
            visc_vec_top(:,i,j,k) = maxval(abs(mat_top(:,:)))
            visc_vec_bot(:,i,j,k) = max(toler, 2.*maxval(abs(mat_top(:,:)))  )
         else if(isotropic==2) then! isotropic viscocity
            visc(:,i,j,k) = max(0.0,  determinant(mat_top,ndim)/max(toler, 2.*determinant(mat_bot,ndim))  )
            visc_vec_top(:,i,j,k) = max(0.0,  determinant(mat_top,ndim))
            visc_vec_bot(:,i,j,k) = max(toler, 2.*determinant(mat_bot,ndim))
         else ! anisotropic (diagonal viscocity)
            do idim=1,ndim
               visc(idim,i,j,k) = max(0.0,  mat_top(idim,idim) / max(toler, 2.*mat_bot(idim,idim))  )
               visc_vec_top(idim,i,j,k) = max(0.0,  abs(mat_top(idim,idim)))
               visc_vec_bot(idim,i,j,k) = max(toler, 2.*abs(mat_bot(idim,idim)))
            end do 
         endif
      end do
      end do
...
! 
      max_bot = maxval(visc_vec_bot(:,:,:,:)) 
      do k=1,nz
      do j=1,ny
      do i=1,nx
         do idim=1,ndim
            visc(idim,i,j,k) = visc_vec_top(idim,i,j,k) &
                             / max( toler, toler_visc*max_bot, visc_vec_bot(idim,i,j,k) )
         end do
      end do
      end do
      end do
